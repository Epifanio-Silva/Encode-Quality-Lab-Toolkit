    ...
    def to_markdown(self) -> str:
        ...
        lines.extend(
            [
                ...
                f"- Frame rate target: `{rec.get('target_frame_rate_raw') or rec.get('target_frame_rate') or 'source'}` via `{rec.get('frame_rate_mode', 'source')}`",
                f"- Color target: `{rec.get('output_color_space', 'source')}` via `{rec.get('color_mode', 'preserve')}`",
                f"- Aspect policy: `{rec.get('aspect_policy', 'fit')}`",
